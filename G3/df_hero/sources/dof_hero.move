...
public fun fight(_admin_cap: &AdminCap, arena: FightArena) {
    ...
    let hero1 = hero_addr1.map!(|addr| {dof::remove<address, Hero>(&mut arena_id, addr)
    }).destroy_some(); 
    ...
    let Hero {id: loser_id, name: _, power : _} = loser_hero; 
    loser_id.delete(); 
    transfer::public_transfer(winner_hero, winner_address); 
}
